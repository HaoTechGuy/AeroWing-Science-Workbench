...
这份手册写给第一次使用 InternAgents 的用户。你可以把它当作一份“照着点”的操作说明：先进入工作台，再放入资料、发起任务、查看结果，最后根据需要配置模型、技能、远程服务和更新。
...
11. [配置页面](#配置页面)
12. [技能](#技能)
13. [连接远程 Agent 服务](#连接远程-agent-服务)
14. [关于、导览和更新](#关于导览和更新)
15. [常见问题](#常见问题)
16. [推荐新手流程](#推荐新手流程)
...
第一次进入工作台时，会出现快速导览。导览会依次介绍当前项目、工作区、聊天区、会话列表、连接服务器、配置和更新。
...
- `连接服务器`：连接本地或远程 LangGraph / InternAgents 服务。
...
![连接服务器](assets/user-manual/05-connect.png)

这里配置的是 Agent 服务连接，不是 SSH 登录。
