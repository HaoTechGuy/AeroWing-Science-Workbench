...
第一次打开时，如果本机还没有模型凭证，会先进入初始配置页面。这里只需要填写 OpenAI 兼容接口的 Base URL、API Key 和模型 ID。

你可以这样操作：

1. 按服务商提供的信息填写模型配置。
2. 点击完成设置。
3. 等待本机后台重新加载配置并进入工作台。

后续如果需要更换模型、API key、工作区或授权模式，可以从工作台进入 `配置` 页面调整。
