...
这是模型服务的钥匙。打开 `OpenRouter 直连` 后，把你的 OpenRouter API key 粘贴到 `OpenRouter API key` 输入框里。
...
- 开启 `OpenRouter 直连`
- `填写 OpenRouter 模型 ID` 可以填 `openrouter/auto`
...
页面里的「集思模型」适合已经接入集思模型服务的环境。普通用户不知道怎么选时，保持 `OpenRouter 直连` 就好。
...
2. 填 OpenRouter API key。
3. 模型先填 `openrouter/auto`。
